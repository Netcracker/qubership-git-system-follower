    ...
    for i, package in enumerate(packages.install, 1):
        ...
        try:
            ...
            if package_state is not None and 'structure_type' in package_state:
                state.add_package(
                    package, extras, response, package_state, structure_type=package_state['structure_type'], source=source)
            else:
                state.add_package(
                    package, extras, response, package_state, structure_type=get_gear_info(package['path'])['structure_type'],
                    source=source)
